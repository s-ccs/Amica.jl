...
!$OMP & PRIVATE (thrdnum,tblksize,t,h,i,j,k,xstrt,xstp,bstrt,bstp,LLinc,tmpsum,usum,vsum)
        ...
        do h = 1,num_models
           ...
           do i = 1,nw
              ...
              !            ' log sbeta(', j, ') = ', log(sbeta(j,comp_list(i,h))), ' log alpha(', j, ') = ', &
              !            log(alpha(j,comp_list(i,h))), ' mu = ', mu(j,comp_list(i,h)); call flush(6)
              !   end if
              !end do
              ...
              !--- get normalized z -- hack to avoid all z == 0 and NaN updates
              do j = 1,num_mix
        ...
        !!$OMP END CRITICAL
           ...
           do i = 1,nw
              ...
              if (update_A .and. do_newton) then
                 ...
                 !!$OMP END CRITICAL
                 dsigma2_numer_tmp_t(i,h,thrdnum+1) = sum( v(bstrt:bstp,h) * b(bstrt:bstp,i,h) * b(bstrt:bstp,i,h) )
                 dsigma2_denom_tmp_t(i,h,thrdnum+1) = vsum
              end if
              !print *, myrank+1,':', thrdnum+1,': getting dc ...'; call flush(6)
              if (update_c) then
                 if (do_reject) then
                    dc_numer_tmp_t(i,h,thrdnum+1) = sum( v(bstrt:bstp,h) * dataseg(seg)%data(i,dataseg(seg)%goodinds(xstrt:xstp)) )
                 else
                    dc_numer_tmp_t(i,h,thrdnum+1) = sum( v(bstrt:bstp,h) * dataseg(seg)%data(i,xstrt:xstp) )
                 end if
                 dc_denom_tmp_t(i,h,thrdnum+1) = vsum
                 !!$OMP CRITICAL
              ...
              do j = 1,num_mix
                 ...
                 !if (.not.(any(.not.(v(bstrt:bstp,h)==dble(0.0))))) then
                 !   print *, 'All v == 0.0!, h = ', h; call flush(6)
                 !end if
                 !if (.not.(any(.not.(z(bstrt:bstp,i,j,h)==dble(0.0))))) then
                 !   print *, 'All z == 0.0!, rank=', myrank+1, ':', thrdnum+1, ' h = ', h, ' i = ', i, ' j = ', j; call flush(6)
                 !end if
                 !if (any(isnan(v(bstrt:bstp,h)))) then
                 !   print *, myrank+1, thrdnum+1, 'h = ', h, ': NaN in v!'; call flush(6)
                 !end if
                 !if (any(isnan(z(bstrt:bstp,i,j,h)))) then
                 !   print *, myrank+1, thrdnum+1, 'h = ', h, ' i = ', i, ' j = ', j, ': NaN in z!'; call flush(6)
                 !end if
               ...
               tmpvec2(bstrt:bstp) = exp((rho(j,comp_list(i,h))-dble(0.0))*tmpvec(bstrt:bstp))
                    ...
                    !if (any(isnan(fp(bstrt:bstp)))) then
                    !   print *, myrank+1, thrdnum+1, ' h = ', h, ' i = ', i, ' j = ', j, ': NaN in fp!'; call flush(6)
                    !end if
                    !if (any(isnan(y(bstrt:bstp,i,j,h)))) then
                    !   print *, myrank+1, thrdnum+1, ' h = ', h, ' i = ', i, ' j = ', j, ': NaN in y!'; call flush(6)
                    !end if
                    !if ((myrank==0) .and. any(isnan(tmpvec(bstrt:bstp)))) then
                       !print *, myrank+1, thrdnum+1, ': NaN in tmpvec!'; call flush(6)
                       !print *, myrank+1, thrdnum+1, ': rho = ', rho(j,comp_list(i,h))
                    !end if
                    !if (any(isnan(tmpvec2(bstrt:bstp)))) then
                       !print *, myrank+1, thrdnum+1, ': NaN in tmpvec2!'; call flush(6)
                       !print *, myrank+1, thrdnum+1, ': rho = ', rho(j,comp_list(i,h))
                    !end if
                 end if
                 ...
                 if (update_A) then
                    ...
                    if (do_newton .and. (iter .ge. newt_start)) then
                       ...
                       !!$OMP END CRITICAL
                       tmpvec(bstrt:bstp) = fp(bstrt:bstp) * y(bstrt:bstp,i,j,h) - dble(1.0)
                       dlambda_numer_tmp_t(j,i,h,thrdnum+1) = sum( u(bstrt:bstp) * tmpvec(bstrt:bstp) * tmpvec(bstrt:bstp) )
                       dlambda_denom_tmp_t(j,i,h,thrdnum+1) = usum
                       dbaralpha_numer_tmp_t(j,i,h,thrdnum+1) = usum
                       dbaralpha_denom_tmp_t(j,i,h,thrdnum+1) = vsum
                    end if
                 end if
                 ...
                 if (update_alpha) then
                     ...
                     !!$OMP END CRITICAL
                    dalpha_numer_tmp_t(j,comp_list(i,h),thrdnum+1) = usum
                    dalpha_denom_tmp_t(j,comp_list(i,h),thrdnum+1) = vsum
                 end if
                 if (update_mu) then
                 ...
                 if (update_beta) then
                    ...
                    !!$OMP END CRITICAL
                    dbeta_numer_tmp_t(j,comp_list(i,h),thrdnum+1) = usum
                    if (rho(j,comp_list(i,h)) .le. dble(2.0)) then
           ...
           if (update_A) then
              ...
              !!$OMP END CRITICAL
              call DAXPY(nw*nw,dble(1.0),Wtmp2(:,:,thrdnum+1),1,dWtmp_t(:,:,h,thrdnum+1),1)
           end if

           if (.false.) then
           do j = 1,num_mix
              if (any(isnan(dmu_numer_tmp(j,:)))) then
                 print *, 'NaN in dmu_numer!'; call flush(6)
              end if
              if (any(isnan(dmu_denom_tmp(j,:)))) then
                 print *, 'NaN in dmu_denom!'; call flush(6)
              end if
              if (any(isnan(dbeta_numer_tmp(j,:)))) then
                 print *, 'NaN in dbeta_numer!'; call flush(6)
              end if
              if (any(isnan(dbeta_denom_tmp(j,:)))) then
                 print *, 'NaN in dbeta_denom!'; call flush(6)
              end if
              if (any(isnan(dalpha_numer_tmp(j,:)))) then
                 print *, 'NaN in dalpha_numer!'; call flush(6)
              end if
              if (any(isnan(dalpha_denom_tmp(j,:)))) then
                 print *, 'NaN in dalpha_denom!'; call flush(6)
              end if
              if (any(isnan(drho_numer_tmp(j,:)))) then
                 print *, 'NaN in drho_numer!'; call flush(6)
              end if
              if (any(isnan(drho_denom_tmp(j,:)))) then
                 print *, 'NaN in drho_denom!'; call flush(6)
              end if
           end do
           end if
...
!$OMP END PARALLEL
        ...
        LLtmp = LLtmp + sum(LLtmp_t)
        dgm_numer_tmp = dgm_numer_tmp + sum(dgm_numer_tmp_t,2)

        dc_numer_tmp = dc_numer_tmp + sum(dc_numer_tmp_t,3)
        dc_denom_tmp = dc_denom_tmp + sum(dc_denom_tmp_t,3)

        dsigma2_numer_tmp = dsigma2_numer_tmp + sum(dsigma2_numer_tmp_t,3)
        dsigma2_denom_tmp = dsigma2_denom_tmp + sum(dsigma2_denom_tmp_t,3)

        dkappa_numer_tmp = dkappa_numer_tmp + sum(dkappa_numer_tmp_t,4)
        dkappa_denom_tmp = dkappa_denom_tmp + sum(dkappa_denom_tmp_t,4)
        dlambda_numer_tmp = dlambda_numer_tmp + sum(dlambda_numer_tmp_t,4)
        dlambda_denom_tmp = dlambda_denom_tmp + sum(dlambda_denom_tmp_t,4)
        dbaralpha_numer_tmp = dbaralpha_numer_tmp + sum(dbaralpha_numer_tmp_t,4)
        dbaralpha_denom_tmp = dbaralpha_denom_tmp + sum(dbaralpha_denom_tmp_t,4)

        dalpha_numer_tmp = dalpha_numer_tmp + sum(dalpha_numer_tmp_t,3)
        dalpha_denom_tmp = dalpha_denom_tmp + sum(dalpha_denom_tmp_t,3)
        dmu_numer_tmp = dmu_numer_tmp + sum(dmu_numer_tmp_t,3)
        dmu_denom_tmp = dmu_denom_tmp + sum(dmu_denom_tmp_t,3)
        dbeta_numer_tmp = dbeta_numer_tmp + sum(dbeta_numer_tmp_t,3)
        dbeta_denom_tmp = dbeta_denom_tmp + sum(dbeta_denom_tmp_t,3)
        drho_numer_tmp = drho_numer_tmp + sum(drho_numer_tmp_t,3)
        drho_denom_tmp = drho_denom_tmp + sum(drho_denom_tmp_t,3)

        dWtmp = dWtmp + sum(dWtmp_t,4)
     ...
     if (.false.) then
        if (myrank == 0) then
           do j = 1,num_mix
              ...
              if (any(isnan(dmu_denom(j,:)))) then
                 print *, 'NaN in dmu_denom!'; call flush(6)
              end if
              if (any(isnan(dmu_numer(j,:)/dmu_denom(j,:)))) then
                 print *, 'NaN in dmu_numer/dmu_denom!'; call flush(6)
              end if
              if (any(isnan(dbeta_numer(j,:)))) then
                 print *, 'NaN in dbeta_numer!'; call flush(6)
              end if
              if (any(isnan(dbeta_denom(j,:)))) then
                 print *, 'NaN in dbeta_denom!'; call flush(6)
              end if
              if (any(isnan(dbeta_numer(j,:)/dbeta_denom(j,:)))) then
                 print *, 'NaN in dbeta_numer/dbeta_denom!'; call flush(6)
              end if
              if (any(isnan(dalpha_numer(j,:)))) then
                 print *, 'NaN in dalpha_numer!'; call flush(6)
              end if
              if (any(isnan(dalpha_denom(j,:)))) then
                 print *, 'NaN in dalpha_denom!'; call flush(6)
              end if
              if (any(isnan(dalpha_numer(j,:)/dalpha_denom(j,:)))) then
                 print *, 'NaN in dalpha_numer/dalpha_denom!'; call flush(6)
              end if
              if (any(isnan(drho_numer(j,:)))) then
                 print *, 'NaN in drho_numer!'; call flush(6)
              end if
              if (any(isnan(drho_denom(j,:)))) then
                 print *, 'NaN in drho_denom!'; call flush(6)
              end if
              if (any(isnan(drho_numer(j,:)/drho_denom(j,:)))) then
                 print *, 'NaN in drho_numer/drho_denom!'; call flush(6)
              end if
           end do
        end if
     end if
...
subroutine update_params
  ...
  if (seg_rank == 0) then
     ...
     if (update_alpha) then
        ...
        !do j = 1,num_mix
        !   if (any(isnan(alpha(j,:)))) then
        !      print *, 'NaN in alpha!'; call flush(6)
        !   end if
        !end do
     end if
     ...
     if (update_A) then
        ...
        if (.false.) then
           do j = 1,num_mix
              ...
              if (any(isnan(dmu_denom(j,:)))) then
                 print *, 'NaN in dmu_denom 2!'; call flush(6)
              end if
              if (any(isnan(dmu_numer(j,:)/dmu_denom(j,:)))) then
                 print *, 'NaN in dmu_numer/dmu_denom 2!'; call flush(6)
              end if
              if (any(isnan(dbeta_numer(j,:)))) then
                 print *, 'NaN in dbeta_numer 2!'; call flush(6)
              end if
              if (any(isnan(dbeta_denom(j,:)))) then
                 print *, 'NaN in dbeta_denom 2!'; call flush(6)
              end if
              if (any(isnan(dbeta_numer(j,:)/dbeta_denom(j,:)))) then
                 print *, 'NaN in dbeta_numer/dbeta_denom 2!'; call flush(6)
              end if
              if (any(isnan(dalpha_numer(j,:)))) then
                 print *, 'NaN in dalpha_numer 2!'; call flush(6)
              end if
              if (any(isnan(dalpha_denom(j,:)))) then
                 print *, 'NaN in dalpha_denom 2!'; call flush(6)
              end if
              if (any(isnan(dalpha_numer(j,:)/dalpha_denom(j,:)))) then
                 print *, 'NaN in dalpha_numer/dalpha_denom 2!'; call flush(6)
              end if
              if (any(isnan(drho_numer(j,:)))) then
                 print *, 'NaN in drho_numer 2!'; call flush(6)
              end if
              if (any(isnan(drho_denom(j,:)))) then
                 print *, 'NaN in drho_denom 2!'; call flush(6)
              end if
              if (any(isnan(drho_numer(j,:)/drho_denom(j,:)))) then
                 print *, 'NaN in drho_numer/drho_denom 2!'; call flush(6)
              end if
           end do
        end if
     ...
     if (update_mu) then
        ...
        !do j = 1,num_mix
        !   do k = 1,num_comps
        !      if (isnan(dmu_numer(j,k)/dmu_denom(j,k))) then
        !         print *, 'dmu: numer = ', dmu_numer(j,k), ' denom = ', dmu_denom(j,k); call flush(6)
        !      end if
        !   end do
     ...
     if (update_beta) then
        !do j = 1,num_mix
        !   do k = 1,num_comps
        !      if (isnan(dbeta_numer(j,k)/dbeta_denom(j,k))) then
        !         print *, 'dmu: numer = ', dbeta_numer(j,k), ' denom = ', dbeta_denom(j,k); call flush(6)
        !      end if
        !   end do
        ...
        do j = 1,num_mix
           if (any(isnan(sbeta(j,:)))) then
              print *, 'NaN in sbeta!'; call flush(6)
           end if
        end do
     end if
